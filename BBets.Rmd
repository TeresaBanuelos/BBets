...
We will approach this problem through a sequence of simulations. First, we will let the number from 1 to 365 represent the different possible  bdays a student could have (1 would be Jan 1ar, 32 would be FEb. 1st etc.) Then we will finally randomly select (with replacement to ensure two of the sme numbers could be chosen.) 35 numbers to represent the same class Then we need to examen the numbers chosen to se if any are duplicates. (a duplicate would represent two people with the same bday.) If there is a duplicate we will give Brownlow $1 for this bet. If not we will take 2 dollars away from him. We will do the same procedure 1137 times. In the end we will totalhis winnings and loses and we will use that number as an estimate of his take-home.
...
Now we just pick 35 of them from a class and loop through 1137 classes keeping score
